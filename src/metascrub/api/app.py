...
def create_app(*, output_dir: str = "./metascrub_cleaned", max_workers: int = 2,
               max_pending: int = 50) -> FastAPI:
    """MetaScrub REST API — POST files, poll the job, pull the cleaned
    files + report as a zip. No built-in authentication: fine on a trusted
    machine/network, but put a reverse proxy (API key / mTLS) in front
    before exposing it. Every job runs in a bounded background thread pool
    so POST returns immediately; max_pending caps queued+running jobs.
    """
    ...
    async def create_clean(
        ...
        # Read + stage uploads now (request body isn't available inside the
    ...
    @app.get("/v1/clean", response_model=list[JobStatusResponse], tags=["clean"])
    def list_jobs(request: Request) -> list[JobStatusResponse]:
        return [_status(j, request) for j in store.list()]

    @app.get("/v1/clean/{job_id}", response_model=JobStatusResponse, tags=["clean"], name="get_job")
    def get_job(job_id: str, request: Request) -> JobStatusResponse:
        return _status(_require(store, job_id), request)

    @app.get("/v1/clean/{job_id}/log", response_model=JobLogResponse, tags=["clean"], name="get_job_log")
    ...
    @app.get("/v1/clean/{job_id}/report.json", tags=["clean"], name="get_job_report_json")
