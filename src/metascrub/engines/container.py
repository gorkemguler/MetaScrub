...
email. Each member MetaScrub understands is scrubbed with the normal
engine; everything else is copied through untouched. Opt-in via
`metascrub clean --recurse`.
...
`.7z` (optional `py7zr`, `pip install 'metascrub[archive]'`); `.msg`
...
class ContainerEngine:
    ...
    def strip(self, src: str, dst: str, cfg: CleanConfig):
        ...
        if kind == "7z" and not _have_py7zr():
            return new_result(src, None, self.name, "skipped",
                              reason="need py7zr for .7z — pip install 'metascrub[archive]'")
...
def _sevenz_members(path: str):
    with tempfile.TemporaryDirectory(prefix="metascrub-7z-") as tmp:
...
def _msg_rows(path: str) -> list[FieldChange]:
    try:
        ...
        return [FieldChange("msg", "note", "install extract-msg to inspect .msg — pip install 'metascrub[msg]'")]
    rows: list[FieldChange] = []
