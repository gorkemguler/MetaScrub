...
# MetaCLS's own mark — "Slant" figlet, distinct from MetaScout's block
...
def _load_project_config() -> dict:
    """Find `.metacls.toml` in the cwd or a parent and turn each table
    ...
    here = os.getcwd()
    while True:
        candidate = os.path.join(here, ".metacls.toml")
...
@click.group()
@click.version_option(package_name="metacls")
...
@click.option("--out", "output_dir", default="./metacls_cleaned", show_default=True,
    ...
    if check:
        ...
        if dirty:
            console.print(
                ...
                f"({report.fields_removed} field(s)).[/bold yellow] Run `metacls clean` on them."
            )
            sys.exit(3)
...
def watch(ctx, directory, filetypes, recursive, to_dir, move_processed, patterns, jobs, interval,
    ...
    DIRECTORY/.metacls-watch.json so a restart doesn't re-scrub
...
@click.option("--api-key", envvar="METACLS_API_KEY", default=None,
...
def api(host, port, output_dir, max_workers, max_pending, api_key, max_upload_mb, max_files,
    ...
    """Launch the MetaCLS REST API (job-based). Requires: pip install 'metacls[api]'."""
    try:
        import uvicorn
        ...
        console.print("  pip install 'metacls[api]'")
        sys.exit(1)
