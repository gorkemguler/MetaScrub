"""`metacls watch` — keep a drop directory scrubbed.
...
A lock file (`.metacls-watch.lock`, holding the owner PID) keeps two
...
_STATE_NAME = ".metacls-watch.json"
_LOCK_NAME = ".metacls-watch.lock"
...
class WatchLockError(RuntimeError):
    """Raised when another live `metacls watch` already owns the directory."""
        ...
        for _ in range(2):
            try:
                ...
                raise WatchLockError(
                    f"another metacls watch is already running on {os.path.dirname(self.path)!r} "
...
class Watcher:
    ...
    def _prune_state(self) -> bool:
        ...
        watcher's `.metacls-watch.json` grows without bound."""
